...
const askForStart = () => {
  let askForStarting;
  console.log(
    `Si no sabes la respuesta de una pregunta puedes poner "pasapalabra" para saltar a la siguiente y escribir "end" para acabar la partida.`
  );
  do {
...
const highScoreTable = () => {
  const highScore = [
    ...
    { playerName: actualPlayerName, score: aciertoshighscore },
  ];
  const highScoreDefinitiva = highScore.sort((a, b) => b.score - a.score);
...
const throwQuestion = () => {
  for (let i = 0; i < questionsGame.length; i++) {
    if (questionsGame[i].status === 0 || questionsGame[i].status === 2) {
      ...
      const actualQuestion = prompt(`${questionsGame[i].question}`);
      ...
      if (actualQuestion === null) {
        ...
        console.log(
          `Has acabado con ${counterGoodQuestions(questionsGame)} aciertos`
        );
        return;
      } else {
        questionsGame[i].status = 3;
...
const mainExecuterFunction = () => {
  ...
  if (counterGoodQuestions(questionsGame) === 27) {
    ...
    console.log(
      `Has completado el rosco con ${counterGoodQuestions(
        questionsGame
      )} aciertos y ${counterWrongQuestions(questionsGame)} fallos.`
    );
    console.log(`Esta es la tabla de puntuaciones!`);
    highScoreTable();
  }
